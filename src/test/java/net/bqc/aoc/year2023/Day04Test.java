...
public class Day04Test extends AbstractTest {
    ...
    void testParseCardPoint(String inputLine, int expectedSum) {
        int computedSum = solution.parseCardPoint(inputLine).winningPoints;
        assertEquals(expectedSum, computedSum);
    }
    ...
    static Stream<Arguments> inputDataSource() {
        ...
        return Stream.of(
            Arguments.of(Solution.PART_NUMBER.ONE, sampleInputLines, 13),
            Arguments.of(Solution.PART_NUMBER.TWO, sampleInputLines, 30),

            Arguments.of(Solution.PART_NUMBER.ONE, inputLines, 24542),
            Arguments.of(Solution.PART_NUMBER.TWO, inputLines, 8736438)
        );
    }
}
